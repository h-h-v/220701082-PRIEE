...
public class PrieeController {
    ...
    @GetMapping("/view")
public String view(@RequestParam("state") String state, Model model) {
    factory fac = new factory();
    ...
    try {
        while (rs2.next()) {
            ...
            String laneValue = rs2.getString("lane"); // e.g., "l1", "l2"
            
            // Extract number and calculate production
            int laneNumber = Integer.parseInt(laneValue.replaceAll("[^0-9]", "")) * 10;
            int production = (laneNumber * 1000) / 100; // Normalize out of 1000
            
            lane.put("lane", laneValue); // Keep original lane name
            lane.put("production", production); // Scaled value
            lanes.add(lane);
        }
    } catch (SQLException e) {
        e.printStackTrace();
    ...
    model.addAttribute("lanes", lanes); // Pass the inventory list
